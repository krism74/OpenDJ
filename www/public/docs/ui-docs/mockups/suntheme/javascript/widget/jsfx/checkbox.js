...
dojo.require("webui.suntheme.widget.checkbox");

/**
 * This function is used to obtain data asynchronously.
 */
webui.suntheme.widget.jsfx.checkbox = {
    /**
    ...
    processRefreshEvent: function(props) {
        ...
        new DynaFaces.fireAjaxTransaction(
            ...
            render: props.id,
            replaceElement: webui.suntheme.widget.jsfx.checkbox.refreshCallback,
            xjson: {
                id: props.id,
                event: "refresh"
            }
    ...
    refreshCallback: function(id, content, closure, xjson) {
        ...
        webui.suntheme.widget.checkbox.refresh.publishEndEvent(json);
        return true;
    }
}

// Listen for Dojo Widget events.
dojo.event.topic.subscribe(webui.suntheme.widget.checkbox.refresh.beginEventTopic,
    webui.suntheme.widget.jsfx.checkbox, "processRefreshEvent");
