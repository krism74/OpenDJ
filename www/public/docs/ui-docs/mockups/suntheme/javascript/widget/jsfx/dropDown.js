...
dojo.require("webui.suntheme.widget.dropDown");

/**
 * This function is used to obtain data asynchronously.
 */
webui.suntheme.widget.jsfx.dropDown = {
    /**
    ...
    processRefreshEvent: function(props) {
        ...
        new DynaFaces.fireAjaxTransaction(
            ...
            render: props.id,
            replaceElement: webui.suntheme.widget.jsfx.dropDown.refreshCallback,
            xjson: {
                id: props.id,
                event: "refresh"
            }
    ...
    refreshCallback: function(id, content, closure, xjson) {
        ...
        webui.suntheme.widget.dropDown.refresh.publishEndEvent(json);
        return true;
    }
}

// Listen for Dojo Widget events.
dojo.event.topic.subscribe(webui.suntheme.widget.dropDown.refresh.beginEventTopic,
    webui.suntheme.widget.jsfx.dropDown, "processRefreshEvent");
